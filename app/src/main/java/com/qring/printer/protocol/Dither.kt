...
enum class DitherMode(val code: Int) {
    ...
    SIERRA_LITE(6),
    /** Burkes：Stucki 的两行简化版，速度快 */
    BURKES(7),
    /** Sierra（三行）：介于 Floyd 和 Jarvis 之间 */
    SIERRA(8),
    /** Bayer 8x8 有序抖动：网点更细，渐变更平滑 */
    BAYER_8X8(9);
}
...
val DITHER_OPTIONS: List<DitherOption> = listOf(
    ...
    DitherOption(DitherMode.BURKES, "Burkes", "Burkes · Stucki 简化版，速度快"),
    DitherOption(DitherMode.SIERRA, "Sierra", "Sierra · 介于 Floyd 和 Jarvis 之间"),
    DitherOption(DitherMode.BAYER_8X8, "Bayer8x8", "有序抖动 8x8 · 网点更细，渐变平滑"),
)
...
/** Bayer 8x8 阈值矩阵 */
private val BAYER_8X8_MATRIX = arrayOf(
...
fun ditherToBinary(gray: GrayImage, mode: DitherMode, threshold: Int): ByteArray {
    ...
    if (mode == DitherMode.BAYER_8X8) {
        for (y in 0 until height) {
            val row = y and 7
            for (x in 0 until width) {
                val t = (th + ((BAYER_8X8_MATRIX[row][x and 7] + 0.5f) / 64f - 0.5f) * 255f).toInt().coerceIn(0, 255)
                out[y * width + x] = if (gray.data[y * width + x] < t) 1 else 0
            }
        }
        return out
    ...
    // 阈值偏移：将 threshold 的变化映射为亮度偏移，使调节效果更明显
    // threshold < 128 → 亮度增加（偏白），threshold > 128 → 亮度降低（偏黑）
    val brightnessShift = (128 - th).toFloat()
    for (i in 0 until total) {
        buffer[i] = (gray.data[i] + brightnessShift).coerceIn(0f, 255f)
    }

    // 误差扩散使用固定中点 128，亮度偏移已在上方应用
    val quantThreshold = 128f

    for (y in 0 until height) {
        for (x in 0 until width) {
            val index = y * width + x
            val oldValue = buffer[index]
            val newValue = if (oldValue < quantThreshold) 0f else 255f
            ...
            when (mode) {
                ...
                DitherMode.BURKES -> {
                    ...
                    if (x + 1 < width) buffer[index + 1] += error * 8f / 32f
                    if (x + 2 < width) buffer[index + 2] += error * 4f / 32f
                    if (y + 1 < height) {
                        if (x > 1) buffer[index + width - 2] += error * 2f / 32f
                        if (x > 0) buffer[index + width - 1] += error * 4f / 32f
                        buffer[index + width] += error * 8f / 32f
                        if (x + 1 < width) buffer[index + width + 1] += error * 4f / 32f
                        if (x + 2 < width) buffer[index + width + 2] += error * 2f / 32f
                    }
                }
                DitherMode.SIERRA -> {
                    //        X  5/32 3/32
                    ...
                    //      2/32 3/32 2/32
                    if (x + 1 < width) buffer[index + 1] += error * 5f / 32f
                    if (x + 2 < width) buffer[index + 2] += error * 3f / 32f
                    if (y + 1 < height) {
                        if (x > 1) buffer[index + width - 2] += error * 2f / 32f
                        if (x > 0) buffer[index + width - 1] += error * 4f / 32f
                        buffer[index + width] += error * 5f / 32f
                        if (x + 1 < width) buffer[index + width + 1] += error * 4f / 32f
                        if (x + 2 < width) buffer[index + width + 2] += error * 2f / 32f
                    }
                    if (y + 2 < height) {
                        if (x > 0) buffer[index + 2 * width - 1] += error * 2f / 32f
                        buffer[index + 2 * width] += error * 3f / 32f
                        if (x + 1 < width) buffer[index + 2 * width + 1] += error * 2f / 32f
                    }
                }
                DitherMode.NONE, DitherMode.BAYER_4X4, DitherMode.BAYER_8X8 -> { /* 已在上方处理 */ }
            }
        }
    }
